...
N_FOLDS = 4
AUDIO_DIR = os.path.join(main,"features/train_set/data/")
SAMPLE_RATE = 44100
LEN_SEC = 300
...
model = STAIMBEnet_v2(NUM_SAMPLES,n_class=len(__class_labels))
print(model)
...
for fold, (train_ids, val_ids) in enumerate(kfold.split(wet)):
    ...
    writer = SummaryWriter('runs/staimbenet_trainer_{}'.format(timestamp))
    epoch_number = 0
    clip = 0.5
    best_f1 = 0.
    best_er = 1e15
    ...
    for epoch in range(max_epochs):
        ...
        with torch.no_grad():
            for j, vdata in enumerate(valloader):
                vmbe,vstai, vlabels = vdata
                voutputs = model.forward(vmbe,vstai)
        ...
        if F1:
            if vf1 > best_f1:
                ...
                model_path = 'models/staimbe_model_{}.pt'.format(timestamp)
                if device == 'cuda':
                    model.cpu()
