...
N_FOLDS = 4
AUDIO_DIR = os.path.join(main,"features/train_set/tut/")
SAMPLE_RATE = 44100
LEN_SEC = 300
...
for fold, (train_ids, val_ids) in enumerate(kfold.split(wet)):
    ...
    writer = SummaryWriter('runs/tutnet_trainer_{}'.format(timestamp))
    epoch_number = 0
    clip = 0.5
    best_f1 = 0.
    best_er = 1e15
    ...
    for epoch in range(max_epochs):
        ...
        if F1:
            if vf1 > best_f1:
                ...
                model_path = 'models/tut_model_{}.pt'.format(timestamp)
                if device == 'cuda':
                    model.cpu()
